...
var betterQueue = require('better-queue');
var fs = require('fs');
var path = require('path');
var xhr2 = require('xhr2');
var ws = require('ws');
var botiumCore = require('botium-core');
...
var botframeworkDirectlinejs__default = /*#__PURE__*/_interopDefaultLegacy(botframeworkDirectlinejs);
var debug__default = /*#__PURE__*/_interopDefaultLegacy(debug$1);
var formData__default = /*#__PURE__*/_interopDefaultLegacy(formData);
var nodeFetch__default = /*#__PURE__*/_interopDefaultLegacy(nodeFetch);
var betterQueue__default = /*#__PURE__*/_interopDefaultLegacy(betterQueue);
var fs__default = /*#__PURE__*/_interopDefaultLegacy(fs);
var path__default = /*#__PURE__*/_interopDefaultLegacy(path);
var xhr2__default = /*#__PURE__*/_interopDefaultLegacy(xhr2);
var ws__default = /*#__PURE__*/_interopDefaultLegacy(ws);
var botiumCore__default = /*#__PURE__*/_interopDefaultLegacy(botiumCore);

var commonjsGlobal = typeof globalThis !== 'undefined' ? globalThis : typeof window !== 'undefined' ? window : typeof global !== 'undefined' ? global : typeof self !== 'undefined' ? self : {};

const uuidv4 = uuid__default["default"].v4;
const {
  DirectLine,
  ConnectionStatus
} = botframeworkDirectlinejs__default["default"];
const debug = debug__default["default"]('botium-connector-directline3');
const Capabilities = {
...
class BotiumConnectorDirectline3 {
  ...
  async Validate() {
    ...
    this.caps = Object.assign({}, Defaults, lodash__default["default"].pickBy(this.caps, (value, key) => !Object.prototype.hasOwnProperty.call(Defaults, key) || !lodash__default["default"].isString(value) || value !== ''));
    ...
    if (this.caps.DIRECTLINE3_ACTIVITY_VALUE_MAP) {
      if (lodash__default["default"].isString(this.caps.DIRECTLINE3_ACTIVITY_VALUE_MAP)) {
    ...
    if (this.caps.DIRECTLINE3_ACTIVITY_TEMPLATE) {
      if (lodash__default["default"].isString(this.caps.DIRECTLINE3_ACTIVITY_TEMPLATE)) {
  ...
  async Start() {
    debug('Start called');
    this.queue = new betterQueue__default["default"]((input, cb) => {
      input().then(result => cb(null, result)).catch(err => cb(err));
    });
    commonjsGlobal.XMLHttpRequest = xhr2__default["default"];
    commonjsGlobal.WebSocket = ws__default["default"];

    if (debug.enabled) {
      commonjsGlobal.window = Object.assign(commonjsGlobal.window || {}, {
        botchatDebug: true
      });
    }
    ...
    if (this.caps.DIRECTLINE3_GENERATE_USERNAME) {
      ...
      this.me = this.caps.DIRECTLINE3_ACTIVITY_TEMPLATE && lodash__default["default"].get(this.caps.DIRECTLINE3_ACTIVITY_TEMPLATE, 'from.id') || 'me';
    }

    const isValidActivityType = activityType => {
      const filter = this.caps.DIRECTLINE3_HANDLE_ACTIVITY_TYPES;

      if (lodash__default["default"].isString(filter)) {
        return filter.indexOf(activityType) >= 0;
      } else if (lodash__default["default"].isArray(filter)) {
    ...
    this.subscription = this.directLine.activity$.filter(activity => isValidActivityType(activity.type) && activity.from.id !== this.me).subscribe(message => {
      if (this.receivedMessageIds[message.id]) {
        ...
        if (message.type === 'message') {
          ...
          const mapImage = i => ({
            mediaUri: i.url,
            mimeType: mimeTypes__default["default"].lookup(i.url) || 'application/unknown',
            altText: i.alt || i.altText
          });

          const mapMedia = m => ({
            mediaUri: m.url,
            mimeType: mimeTypes__default["default"].lookup(m.url) || 'application/unknown',
            altText: m.profile
          });
          ...
          message.attachments && message.attachments.forEach(a => {
            ...
            } else if (a.contentType === 'application/vnd.microsoft.card.animation' || a.contentType === 'application/vnd.microsoft.card.audio' || a.contentType === 'application/vnd.microsoft.card.video') {
              botMsg.cards.push({
                text: a.content.title || a.content.text,
                subtext: a.content.subtitle,
                content: a.content.text,
          ...
          if (!botMsg.messageText && botMsg.cards) {
            ...
            if (card && lodash__default["default"].isArray(card.text) && card.text.length > 0) {
              botMsg.messageText = card.text[0];
            } else if (card && lodash__default["default"].isString(card.text)) {
    ...
    this.connSubscription = this.directLine.connectionStatus$.subscribe(connectionStatus => {
    ...
    if (this.caps[Capabilities.DIRECTLINE3_WELCOME_ACTIVITY]) {
      if (lodash__default["default"].isString(this.caps[Capabilities.DIRECTLINE3_WELCOME_ACTIVITY])) {
        ...
        if (initActivity) {
          this.UserSays({
            sourceData: initActivity
          }).catch(err => debug(`Failed to send DIRECTLINE3_WELCOME_ACTIVITY: ${err.message}`));
        } else {
          this.UserSays({
            messageText: this.caps[Capabilities.DIRECTLINE3_WELCOME_ACTIVITY]
          }).catch(err => debug(`Failed to send DIRECTLINE3_WELCOME_ACTIVITY: ${err.message}`));
        }
      } else {
        this.UserSays({
          sourceData: this.caps[Capabilities.DIRECTLINE3_WELCOME_ACTIVITY]
        }).catch(err => debug(`Failed to send DIRECTLINE3_WELCOME_ACTIVITY: ${err.message}`));
      }
    } else {
  ...
  async UserSays(msg) {
    ...
    if (msg.buttons && msg.buttons.length > 0 && (msg.buttons[0].text || msg.buttons[0].payload)) {
      ...
      lodash__default["default"].set(activity, this.caps[Capabilities.DIRECTLINE3_BUTTON_VALUE_FIELD], payload);
    } else {
      if (!activity.type) {
        activity.type = 'message';
      }

      if (!lodash__default["default"].isUndefined(msg.messageText)) {
    ...
    if (msg.forms) {
      ...
      msg.forms.forEach(f => {
        lodash__default["default"].set(activity.value, f.name, f.value);
      });
    }

    if (msg.SET_ACTIVITY_VALUE) {
      lodash__default["default"].keys(msg.SET_ACTIVITY_VALUE).forEach(key => {
        lodash__default["default"].set(activity, key, msg.SET_ACTIVITY_VALUE[key]);
      });
    } // validating the activity


    if (activity.text) {
      if (lodash__default["default"].isObject(activity.text)) {
    ...
    if (msg.media && msg.media.length > 0) {
      ...
      msg.sourceData = Object.assign(msg.sourceData || {}, {
        activity
      });
      ...
      for (let i = 0; i < msg.media.length; i++) {
        ...
        const attachmentName = path__default["default"].basename(attachment.mediaUri);
        ...
        if (attachment.buffer) {
          ...
          if (!this.caps[CoreCapabilities.SECURITY_ALLOW_UNSAFE]) {
            throw new BotiumError('Security Error. Illegal configured MediaInput. Sending attachment using the filesystem is not allowed', {
              type: 'security',
          ...
          formData.append('file', fs__default["default"].createReadStream(filepath), {
            filename: attachmentName
          });
        } else if (attachment.downloadUri) {
          const res = await nodeFetch__default["default"](attachment.downloadUri);
      ...
      return this._runInQueue(async () => {
        try {
          const res = await nodeFetch__default["default"](uploadUrl, {
            method: 'POST',
            headers: {
      ...
      return this._runInQueue(() => new Promise((resolve, reject) => {
        this.directLine.postActivity(activity).subscribe(id => {
  ...
  async _runInQueue(fn) {
    if (this.queue) {
      return new Promise((resolve, reject) => {
        this.queue.push(fn).on('finish', resolve).on('failed', reject);
      });
    } else {
  ...
  _deepFilter(item, selectFn, filterFn) {
    ...
    if (lodash__default["default"].isArray(item)) {
...
var botiumConnectorDirectline3 = {
  PluginVersion: 1,
...
var botiumConnectorDirectline3_1 = botiumConnectorDirectline3.PluginVersion;
var botiumConnectorDirectline3_2 = botiumConnectorDirectline3.PluginClass;
var botiumConnectorDirectline3_3 = botiumConnectorDirectline3.PluginDesc;

exports.PluginClass = botiumConnectorDirectline3_2;
exports.PluginDesc = botiumConnectorDirectline3_3;
exports.PluginVersion = botiumConnectorDirectline3_1;
exports["default"] = botiumConnectorDirectline3;
//# sourceMappingURL=botium-connector-directline3-cjs.js.map
