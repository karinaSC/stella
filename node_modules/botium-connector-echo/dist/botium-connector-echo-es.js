...
const debug = debug$1('botium-connector-echo');
const Capabilities = {
...
class BotiumConnectorEcho {
  ...
  Build() {
    if (this.caps[Capabilities.ECHO_ANSWERS]) {
      const extraAnswers = lodash.isString(this.caps[Capabilities.ECHO_ANSWERS]) ? JSON.parse(this.caps[Capabilities.ECHO_ANSWERS]) : this.caps[Capabilities.ECHO_ANSWERS];
      this.answers = this.answers.concat(extraAnswers);

      for (const a of this.answers) {
        if (lodash.isString(a.input)) a.input = [a.input];
      }
    }

    if (this.caps[Capabilities.ECHO_WELCOMEMESSAGE]) {
      const extraWelcomeMessages = lodash.isString(this.caps[Capabilities.ECHO_WELCOMEMESSAGE]) ? JSON.parse(this.caps[Capabilities.ECHO_WELCOMEMESSAGE]) : this.caps[Capabilities.ECHO_WELCOMEMESSAGE];
  ...
  Start() {
    ...
    for (const welcomeMessage of this.welcomeMessages) {
      ...
      if (lodash.isString(welcomeMessage)) {
        botMsg.messageText = welcomeMessage;
      } else if (lodash.isFunction(welcomeMessage.output)) {
        botMsg = Object.assign(botMsg, welcomeMessage.output({}, this.session));
      } else if (lodash.isString(welcomeMessage.output)) {
  ...
  UserSays(msg) {
    ...
    if (template && template.output) {
      if (lodash.isFunction(template.output)) {
        botMsg = Object.assign(botMsg, template.output(msg, this.session));
      } else if (lodash.isString(template.output)) {
...
export { botiumConnectorEcho_4 as Export, botiumConnectorEcho_3 as Import, botiumConnectorEcho_2 as PluginClass, botiumConnectorEcho_5 as PluginDesc, botiumConnectorEcho_1 as PluginVersion, botiumConnectorEcho as default };
//# sourceMappingURL=botium-connector-echo-es.js.map
